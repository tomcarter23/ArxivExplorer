...
def mock_data_handler(mock_sentence_transformer, mock_faiss_index, mock_mongo_collection):
    dh = DataHandler(
        dataset="./tests/data/paper_data.json",
...
def test_data_handler_load_data(mock_data_handler):
    data = [d for d in mock_data_handler.load_data("./tests/data/paper_data.json")]
    assert len(data) == 2
...
def test_data_handler_load_data_n(mock_data_handler):
    data = [d for d in mock_data_handler.load_data("./tests/data/paper_data.json", n=1)]
    assert len(data) == 1
    assert data[0]["title"] == "The Art of Computer Programming"


def test_data_handler_load_data_n0_raises(mock_data_handler):
    with pytest.raises(ValueError):
        _ = [d for d in mock_data_handler.load_data("./tests/data/paper_data.json", n=0)]
...
def test_data_handler_process_and_save(mock_data_handler):
    with patch("faiss.write_index") as mock_write_index:
        mock_data_handler.process_and_save(index_path="index_path")
        assert mock_write_index.call_count == 1
        assert mock_write_index.call_args[0][1] == "index_path"
